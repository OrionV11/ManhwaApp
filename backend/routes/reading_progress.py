...
def add_user_reading_media_route(user_id: int, media_id: int, db: Session = Depends(get_db)):
...
@router.put("/{user_id}/complete/{media_id}", status_code=status.HTTP_200_OK)
def mark_media_completed_route(
    ...
    try:
        return mark_as_completed(  # Changed from reading_progress_controller.mark_as_completed
            db=db,
            user_id=user_id,
            media_id=media_id
        )
